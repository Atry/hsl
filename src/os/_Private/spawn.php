...
function fork_and_execve_addchdir(
...
function fork_and_execve_adddup2(
  _OS\TForkAndExecve $fork_and_execve,
...
function fork_and_execve_addopen(
  _OS\TForkAndExecve $fork_and_execve,
  ...
  return C\reduce(
    ...
    ($fork_and_execve, $file_action) ==> {
      if ($file_action is OS\posix_spawn_file_actions_addchdir_np)
        return _OS\fork_and_execve_addchdir($fork_and_execve, $file_action);

      if ($file_action is OS\posix_spawn_file_actions_adddup2)
        return _OS\fork_and_execve_adddup2($fork_and_execve, $file_action);

      $file_action as OS\posix_spawn_file_actions_addopen;
      return _OS\fork_and_execve_addopen($fork_and_execve, $file_action);
    },
    _OS\fork_and_execve_default<>,
  );
}

// Apply posix_spawnattr_t to _OS\TForkAndExecveOptions
function parse_posix_spawnattr_t(
  _OS\TForkAndExecveOptions $options,
  ...
  if ($posix_spawnattr_setflags is nonnull) {
    $options['setsid'] =
      ($posix_spawnattr_setflags & OS\POSIX_SPAWN_SETSID) !== 0;
    if (($posix_spawnattr_setflags & OS\POSIX_SPAWN_SETPGROUP) !== 0) {
      $options['setpgid'] =
